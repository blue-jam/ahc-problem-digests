...
def test_save_digest_creates_file(tmp_path):
    ...
    path = save_digest("ahc001", "これは要約です。", digests_dir=tmp_path)
...
def test_save_digest_file_contents(tmp_path):
    ...
    save_digest("ahc001", "要約テキスト", digests_dir=tmp_path)
...
def test_save_digest_creates_directory_if_missing(tmp_path):
    ...
    save_digest("ahc002", "要約", digests_dir=nested)
...
def test_load_digest_returns_data(tmp_path):
    ...
    save_digest("ahc003", "要約", digests_dir=tmp_path)
...
def test_save_and_load_roundtrip(tmp_path):
    ...
    save_digest("ahc010", summary, digests_dir=tmp_path)
